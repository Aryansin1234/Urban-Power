...
export default function ProductsSection() {
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLDivElement>, product: Product) => {
  ...
  return (
    <motion.section
      id="products"
      className="relative overflow-hidden bg-gradient-to-br from-gray-50 via-white to-primary/5 py-20 sm:py-28"
      initial="hidden"
      whileInView="visible"
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute -left-32 -top-40 h-64 w-64 rounded-full bg-primary/10 blur-3xl"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -right-24 bottom-0 h-96 w-96 rounded-full bg-emerald-200/40 blur-3xl"
      />

      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <motion.div className="mx-auto mb-16 max-w-3xl text-center" variants={sectionVariants}>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3">
          {products.map((product) => {
            ...
            return (
              <motion.article
                key={product.id}
                className="group relative flex cursor-pointer flex-col items-center gap-5 overflow-hidden rounded-3xl border border-white/60 bg-white/80 p-8 text-center shadow-lg backdrop-blur transition-shadow duration-300 hover:shadow-2xl"
                initial="hidden"
                whileInView="visible"
                whileHover="hover"
                whileTap="tap"
              ...
              >
                ...
                <div className="relative flex flex-col items-center gap-5">
                  <span className="inline-flex items-center gap-2 rounded-full bg-primary/5 px-4 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
                  ...
                  {highlightFeatures.length > 0 && (
                    <div className="flex flex-wrap justify-center gap-2">
                      {highlightFeatures.map((feature) => (
                        <span
                          key={feature}
                          className="inline-flex items-center rounded-full border border-primary/20 bg-white px-3 py-1 text-xs font-medium text-primary/80 shadow-sm"
                        >
                          {feature}
                        </span>
                      ))}
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.96 }}
                    className="mt-auto inline-flex items-center gap-2 rounded-full border border-primary/10 bg-primary/10 px-5 py-2 text-sm font-semibold text-primary transition-colors duration-300 group-hover:border-primary group-hover:bg-primary group-hover:text-white"
                    onClick={(event) => {
        ...
        <AnimatePresence>
          {expandedProduct && (
            ...
            >
              <motion.div
                className="relative w-full max-w-3xl rounded-3xl border border-white/40 bg-gradient-to-br from-white via-white to-primary/10 p-8 shadow-2xl"
              ...
              >
                <button
                  ...
                  className="absolute right-4 top-4 inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 bg-white text-lg text-gray-500 transition hover:scale-105 hover:text-gray-800"
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid gap-6 md">
                      <div className="space-y-4">
                        <div className="flex flex-col items-center gap-3 rounded-2xl border border-primary/10 bg-white/70 p-4 text-center shadow-sm md:items-start md:text-left">
                          <img
                        ...
                        <button
                          className="w-50 mt-4 px-6 py-3 rounded-xl bg-primary text-white font-bold text-lg shadow-lg hover:bg-primary-dark transition"
                          onClick={() => {
                            handleCollapse();
                            setTimeout(() => {
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.97 }}
                      className="inline-flex items-center justify-center gap-2 rounded-full border border-primary/20 bg-white px-6 py-2 text-sm font-semibold text-primary transition hover:bg-primary hover:text-white"
                      ...
                      <div className="grid gap-3">
                        {expandedProduct.models.map((model) => (
                          <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            className="relative flex w-full items-center justify-between overflow-hidden rounded-2xl border border-primary/20 bg-white px-5 py-4 text-left text-sm font-semibold text-gray-800 shadow-sm transition hover:border-primary hover:bg-primary/10"
                          ...
                          >
                            <div className="flex items-center gap-4">
                              <img
                                ...
                                alt={model.name}
                                className="h-16 w-16 flex-shrink-0 rounded-2xl object-cover shadow-sm"
                              />
